...
def connect_to_tv():
    ...
    if current_tv_id != '':
        try:
            # Установка соединения с выбранным устройством TV
            tv_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            tv_socket.settimeout(2)  # Установка тайм-аута для подключения
...
def send_data_to_tv(data):
    ...
    if tv_connection_state == True:
        try:
            ...
            tv_socket.connect((current_tv_ip, current_tv_port))
            # tv_socket.sendall(data.encode())
            tv_socket_command = Devices_Command(tv_socket, data.encode(), 3, current_tv_ip, current_tv_port)
            send_queue.append(tv_socket_command)
            send_queue.process()
            # Здесь можно добавить логику для обработки ответов от устройства TV
            tv_socket.close()
        except:
    ...
    if current_tv_id != '': 
        if tv_btn_id == 42:
            ...
            if tv_connection_state == True:
                ...
                logs_screen.custom_logger('TV power command send to sender from tv_power_control')           
            else:
                logs_screen.custom_logger("Unable t send command. TV not connected. Check connection.")
...
def set_keypad(tv_btn_id):
    ...
    if tv_connection_state == True:
        ...
        if tv_btn_id in ValueStateValues:
            ...
            PowerCmdString = 'mc {0} {1} 20 0D'.format(int(current_tv_id), ValueStateValues[tv_btn_id]['key'])
            send_data_to_tv(PowerCmdString)
...
def menu_navigation(tv_btn_id):
    ...
    if tv_connection_state == True:
        ...
        if tv_btn_id in ValueStateValues: 
            MenuNavigationCmdString = 'mc {0} {1} 20 0D'.format(int(current_tv_id), ValueStateValues[tv_btn_id]['value'])
