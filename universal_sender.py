...
class Devices_Command:
    def __init__(self, receiving_object: object, content, priority: int, ip: str = None, port: int = None):
        ...
        for message in self.queue:
            ...
            if isinstance(receiving_object, socket.socket):
                try:        
                    ...
                    logs_screen.custom_logger('Ethernet interface: Unable to send data', str(e))

            elif isinstance(receiving_object, SerialInterface):
                try:
                    # Если объект является экземпляром класса SerialInterface
                    receiving_object.Send(content)  # Выполняем команду для интерфейса SerialInterface                    
