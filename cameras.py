...
camera_nodes = {
    16: {'address': '10.90.5.51', 'port': 1259, 'protocol': 'UDP'}, #was 5678 TCP and 1259 UDP
...
def init_buttons(webgui: UIDevice):
    ...
    CAM_ACTIONS[127] = Button(webgui, 127, holdTime=None, repeatTime=None)    #Power ON btn
    CAM_ACTIONS[126] = Button(webgui, 126, holdTime=None, repeatTime=None)    #Power OFF btn
    CAM_ACTIONS[15] = Button(webgui, 15, holdTime=None, repeatTime=None)    #Zoom in btn
    CAM_ACTIONS[102] = Button(webgui, 102, holdTime=None, repeatTime=None)    #Zoom out btn
    ...
    CAM_PRESETS_ACT[128] = Button(webgui, 128, holdTime=None, repeatTime=None)  #save preset btn
    CAM_PRESETS_ACT[129] = Button(webgui, 129, holdTime=None, repeatTime=None)  #show preset btn
...
def __SetHelper(PowerCmdString, priority_com):
    ...
    if cam_node_udp != None:
        socket_sender(message=PowerCmdString, ip_address=cam_node_udp['address'], port=int(cam_node_udp['port']), priority=priority_com)
        ...
        logs_screen.custom_logger('Unable to send from camera to socket_sender. Wrond parameters')
...
def socket_sender(message, ip_address, port, priority): #TODO ADD PRIORITY
    ...
    if ip_address == '10.90.5.51':
        try:
            cam1_socket_command = Devices_Command(cam1_socket, message, int(priority), str(ip_address), int(port))
            ...
            logs_screen.custom_logger('Unable to send data to queue from cam_socket 1.', str(e))
    elif ip_address == '10.90.5.52':
        try:
            cam2_socket_command = Devices_Command(cam2_socket, message, int(priority), str(ip_address), int(port))
            ...
            logs_screen.custom_logger('Unable to send data to queue from cam_socket 2.', str(e))
    elif ip_address == '10.90.5.53':
        try:
            cam3_socket_command = Devices_Command(cam3_socket, message, int(priority), str(ip_address), int(port))
            ...
            logs_screen.custom_logger('Unable to send data to queue from cam_socket 3.', str(e))
    else:
...
def reset_cameras_states():
    ...
    for btn in CAM_CONTROL_GRP:
        btn.SetState == 0
  
    for btn in CAM_PRESET_CONTROL_GRP:
        btn.SetState == 0
            ...
            if active_preset_number !=0:
                if preset_btn == 128: #show
                    ...
                    for cam_pr in CAM_PRESET_GRP:
                        PowerCmdString = b'\x81\x01\x04\x3F\x02' + bytes([active_preset_number]) + b'\xFF'
                        __SetHelper(PowerCmdString, 2)
                        logs_screen.custom_logger('Command SHOW PRESET send to socket_sender')
                        logs_screen.custom_logger(PowerCmdString, 2)
                        # print(PowerCmdString)
                elif preset_btn == 129: #save
                    for cam_pr in CAM_PRESET_GRP:
                        PowerCmdString = b'\x81\x01\x04\x3F\x01' + bytes([active_preset_number]) + b'\xFF'
                        __SetHelper(PowerCmdString, 2)
                        # print(PowerCmdString)
                        logs_screen.custom_logger('Command SAVE PRESET send to socket_sender')
                        logs_screen.custom_logger(PowerCmdString, 2)
            else:
                logs_screen.custom_logger('Wrong preset ID')
                print('Wrong preset ID')
